use dv_report_metrics::registry::{Histogram, IntCounter, IntCounterVec, IntGauge};
use once_cell::sync::Lazy;

const METRIC_PREFIX: &str = "dv_report_api_service";

pub(crate) fn request_counter() -> IntCounter {
    static METER: Lazy<IntCounter> = Lazy::new(|| {
        dv_report_metrics::registry::register_int_counter(
            METRIC_PREFIX,
            "request_count",
...
pub(crate) fn response_time_ms() -> Histogram {
    static METER: Lazy<Histogram> = Lazy::new(|| {
        dv_report_metrics::registry::register_histogram(
            METRIC_PREFIX,
            "response_time_ms",
...
pub(crate) fn response_status_code_counter(status_code: &str) -> IntCounter {
    static METER: Lazy<IntCounterVec> = Lazy::new(|| {
        dv_report_metrics::registry::register_int_counter_vec(
            METRIC_PREFIX,
