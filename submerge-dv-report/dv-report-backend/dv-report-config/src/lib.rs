...
impl Config {
    fn new() -> Result<Self, config::ConfigError> {
        let env = Environment::from(
            std::env::var("DV_REPORT_ENV")
                .unwrap_or_else(|_| "Production".into())
                .as_str(),
        );
        let network = std::env::var("DV_REPORT_NETWORK").unwrap_or_else(|_| DEFAULT_NETWORK.into());
        let config_dir = if cfg!(debug_assertions) {
            std::env::var("DV_REPORT_CONFIG_DIR").unwrap_or_else(|_| DEV_CONFIG_DIR.into())
        } else {
            std::env::var("DV_REPORT_CONFIG_DIR").unwrap_or_else(|_| DEFAULT_CONFIG_DIR.into())
        };
        let config = config::Config::builder()
            ...
            .add_source(config::Environment::with_prefix("dv_report").separator("__"))
            .build()?;
