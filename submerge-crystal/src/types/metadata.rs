...
fn get_decode_different_string(value: &DecodeDifferentStr) -> String {
    match value {
...
impl From<&RuntimeMetadataV8> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV8) -> Self {
                ...
                for (index, module) in modules.iter().enumerate() {
                    ...
                    if let Some(module_events) = &module.event {
                        match module_events {
                            DecodeDifferent::Encode(module_events) => {
                                for (index, module_event) in (module_events.0)().iter().enumerate()
                                {
                                    let name = get_decode_different_string(&module_event.name)
                                        .to_case(Case::UpperCamel);
                            ...
                            DecodeDifferent::Decoded(module_events) => {
                                for (index, module_event) in module_events.iter().enumerate() {
                                    let name = get_decode_different_string(&module_event.name)
                                        .to_case(Case::UpperCamel);
                                    pallet.events.push(Event {
                                        index: index as u8,
                    ...
                    match &module.constants {
                        DecodeDifferent::Encode(module_constants) => {
                            ...
                            {
                                let name = get_decode_different_string(&module_constant.name)
                                    .to_case(Case::UpperCamel);
                        ...
                        DecodeDifferent::Decoded(module_constants) => {
                            for (index, module_constant) in module_constants.iter().enumerate() {
                                let name = get_decode_different_string(&module_constant.name)
                                    .to_case(Case::UpperCamel);
                                pallet.constants.push(Constant {
                                    index: index as u8,
                    ...
                    if let Some(storage) = &module.storage {
                        match &storage {
                            DecodeDifferent::Encode(storage) => match &storage.0().entries {
...
impl From<&RuntimeMetadataV9> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV9) -> Self {
        let mut metadata = Metadata::default();
        match &value.modules {
            DecodeDifferent::Decoded(modules) => {
                for (index, module) in modules.iter().enumerate() {
...
impl From<&RuntimeMetadataV10> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV10) -> Self {
        let mut metadata = Metadata::default();
        match &value.modules {
            DecodeDifferent::Decoded(modules) => {
                for (index, module) in modules.iter().enumerate() {
...
impl From<&RuntimeMetadataV11> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV11) -> Self {
        let mut metadata = Metadata::default();
        match &value.modules {
            DecodeDifferent::Decoded(modules) => {
                for (index, module) in modules.iter().enumerate() {
...
impl From<&RuntimeMetadataV12> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV12) -> Self {
        let mut metadata = Metadata::default();
        match &value.modules {
            DecodeDifferent::Decoded(modules) => {
                for (index, module) in modules.iter().enumerate() {
...
impl From<&RuntimeMetadataV13> for Metadata {
    #[allow(clippy::cognitive_complexity)]
    fn from(value: &RuntimeMetadataV13) -> Self {
        let mut metadata = Metadata::default();
        match &value.modules {
            DecodeDifferent::Decoded(modules) => {
                for (index, module) in modules.iter().enumerate() {
...
impl TryFrom<&RuntimeMetadataV14> for Metadata {
    ...
    fn try_from(value: &RuntimeMetadataV14) -> Result<Self, anyhow::Error> {
        let mut metadata = Metadata::default();
...
impl TryFrom<&RuntimeMetadataV15> for Metadata {
    ...
    fn try_from(value: &RuntimeMetadataV15) -> Result<Self, anyhow::Error> {
        let mut metadata = Metadata::default();
        for pallet_metadata in value.pallets.iter() {
            let mut pallet = Pallet {
                index: pallet_metadata.index,
...
impl TryFrom<&RuntimeMetadataV16> for Metadata {
    ...
    fn try_from(value: &RuntimeMetadataV16) -> Result<Self, anyhow::Error> {
        let mut metadata = Metadata::default();
        for pallet_metadata in value.pallets.iter() {
            let mut pallet = Pallet {
                index: pallet_metadata.index,
