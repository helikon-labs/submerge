...
pub(crate) trait CrystalAPIPostgreSQLStorage {
    async fn get_metadata_list(&self) -> anyhow::Result<Vec<Metadata>>;
...
impl CrystalAPIPostgreSQLStorage for PostgreSQLStorage {
    async fn get_metadata_list(&self) -> anyhow::Result<Vec<Metadata>> {
        let rows: Vec<(i32, i32)> = sqlx::query_as(
            "SELECT spec_version, metadata_version FROM metadata ORDER BY spec_version ASC",
        )
